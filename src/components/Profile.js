...
import { useAuth } from "../contexts/AuthContext"; // usa el hook useAuth que ya tenías
...
export default function Profile() {
  ...
  // Límites por defecto (mantengo tus valores originales)
  const defaultUsage = {
    ...
    const fetchProfileAndUsage = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // si no existe doc en Firestore, usamos info mínima desde auth
          data = {
        ...
        // 2) Contar operaciones en la subcolección "users/{uid}/operations"
        ...
        // 3) Normalizar usage en profileData (si no existe)
        if (data?.usage) {
          // si usage existe, pero operaciones.usadas está indefinido, actualizamos con opsCount
        ...
        // actualizar el estado con usage corregido
        setProfileData({ ...data });
      } catch (error) {
  ...
  // Helper para foto: prioriza Firestore -> Auth -> null
  const getPhotoURL = () => {
  ...
  // SVGs inline para evitar dependencia externa (@heroicons) y que compile en build
  ...
  // Valores de uso finales (operaciones / exportaciones)
  ...
  // Fecha de miembro (preferir firestore -> auth)
  const memberSince =
  ...
  return (
    <div className="min-h-screen bg-black text-white py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-indigo-800 to-purple-800 rounded-2xl p-6 mb-8">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-sm text-indigo-100 mt-2">Miembro desde: {memberSince}</p>

              {/* Botones: QUITÉ 'Ajustes de cuenta' tal como pediste */}
          ...
          <div className="space-y-8">
            <div className="bg-gray-900 rounded-xl p-6 shadow-lg">
              <h3 className="text-lg font-semibold mb-4 flex items-center">
                {/* Icono de cohete corregido, no toqué la posición */}
                <IconRocket />
                Plan Premium
              </h3>
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {/* Colores según lo pediste */}
                  <button className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-3 rounded-md">PayPal</button>
                  <button className="bg-yellow-400 hover:bg-yellow-500 text-gray-900 py-2 px-3 rounded-md">Binance Pay</button>
                  <button className="bg-sky-500 hover:bg-sky-600 text-white py-2 px-3 rounded-md">Blockchain Pay</button>
                </div>
              </div>
            ...
            {/* Botón de contacto con soporte (mailto) */}
        ...
        {/* Espacio final para que no pegue el footer/estado */}
        <div className="h-10" />
      </div>
    </div>
  );
